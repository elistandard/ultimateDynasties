...
#import data
df_team_season<-read.csv("ultimate_standings_clean.csv")
ref_teams<-read.csv("team_logos.csv")

# Retain one URL per team
unique_team_urls <- ref_teams %>%
  group_by(team_name) %>%
  slice(1) %>%  # Keep the first row for each team
  ungroup() %>%
  select(team_name, team_url, logo_url)  # Select only relevant columns

# Modify this to calculate runs
team_summary<-df_team_season|>
  select(Team, Rank, Year)|>
  group_by(Team)|>
  summarise(first_season = min(Year),
            last_season = max(Year),
            first_rank = Rank[Year==first_season],
            last_rank = Rank[Year==last_season])|>
  mutate(delta = first_rank-last_rank)|>
  ungroup()|>
  arrange(delta)
...
#ggplot() +
  ...
  #) +
  #plot logos for clubs
  #ggimage::geom_image(data=bump_data, 
